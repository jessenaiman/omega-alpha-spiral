START: Read `project-management/STATUS.md` and `BOARD.md`.
IDENTIFY: Match the current branch/worktree to its task and `Handoffs/<task-id>.md`.
PAUSE: Do not search, grep, inspect code, or edit before confirming the handoff.
CONFIRM: Report task, branch, last result, and next step; ask the user if current.
LEARN: Relevant skills read/write their existing `learnings.md`; update the task handoff.
[DO NOT EDIT LINE 1-5]
---
## RULES FOR ADDING TO THIS
- bullet points, have purpose, like less context `/caveman full|ultra`
- always use threejs* skills and follow those instructions
- `blender mcp helop` write the correct starting command that has the instructions only with (this needs an agent to fix by checking the mcp tool)
- if tool is broken try 1 thing and then alert the user, but check with `ask-matt`

























































































----THIS IS AS FAR AS YOU CAN ADD CONTENT----