...
A knowledge page _is_ a [mental model](./mental-models). Same synthesis, same background refresh, same provenance.
...
That last part matters. Search is a tool an agent _chooses_ to call, visible in the transcript, rather than content pushed into its context on every turn. Retrieval the agent asked for informs what it's doing; retrieval it didn't ask for tends to derail it.
...
Your raw documents remain the source of truth about _what was said_. The pages are the reconciled truth about _what holds_.
