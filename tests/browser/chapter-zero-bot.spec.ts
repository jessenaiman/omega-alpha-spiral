...
declare global { interface Window {
  __CHAPTER_TWO_DIAGNOSTICS__: { getState(): FieldState };
} }
  ...
  test(`keyboard bot route ${route} crosses into playable rooms`, async ({ page }, info) => {
    ...
    for (let question = 0; question < 4; question += 1) {
      await page.evaluate((state) => {
        const hooks = (window as unknown as { __THREE_GAME_TEST_HOOKS__?: { setState(name: string): Promise<{ state: string }> } }).__THREE_GAME_TEST_HOOKS__;
        if (!hooks) throw new Error('Missing intro test hooks');
        return hooks.setState(state);
      }, `question-${question + 1}`);
      ...
      const choice = (route + question) % choiceCount;
      evidence.push({ questionIndex: await page.evaluate(() => {
        const diagnostics = (window as unknown as { __THREE_GAME_DIAGNOSTICS__?: { state: { questionIndex: number } } }).__THREE_GAME_DIAGNOSTICS__;
        if (!diagnostics) throw new Error('Missing intro diagnostics');
        return diagnostics.state.questionIndex;
      }), choice });
    ...
    await page.evaluate(() => {
      const hooks = (window as unknown as { __THREE_GAME_TEST_HOOKS__?: { setState(name: string): Promise<{ state: string }> } }).__THREE_GAME_TEST_HOOKS__;
      if (!hooks) throw new Error('Missing intro test hooks');
      return hooks.setState('final-door');
    });
    await expect(page.locator('main')).toHaveAttribute('data-os-phase-ts', 'doorway');
