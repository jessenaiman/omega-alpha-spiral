...
function driverGuardIntent(seed: string): { ok: boolean; state: FloorState } {
  ...
  if (!s.guard || !adjacentTo(s.guard.pos)(s.player.x, s.player.y)) return { ok: false, state: s };
...
function driverPickup(seed: string): { ok: boolean; state: FloorState } {
  let s = walkTo(createFloor(seed), (st, x, y) => tileAt(st, x, y) === 'pickup');
...
function report(name: string, seed: string, result: { ok: boolean; state: FloorState }): void {
...
console.log('map:', `${FLOOR_ONE_MAP.length} rows x ${FLOOR_ONE_MAP[0]?.length} cols`);
...
if (doorSeed === null) console.log('door-open: NO SEED FOUND in door-1..door-400');
