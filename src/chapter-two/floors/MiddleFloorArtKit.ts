 ...
 */
export function createMiddleFloorArtKit(
  layout: MiddleFloorLayout
): MiddleFloorArtResult {
  ...
  return {
    ...
    aimTelegraph(enemyId: string, target: FloorPoint): boolean {
      ...
      if (!aim || !Number.isFinite(target.x) || !Number.isFinite(target.z))
        return false;
      aim(target);
      return true;
    },
    dispose(): void {
      ...
      root.traverse((object: THREE.Object3D): void => {
        ...
        const collectMaterial = (material: THREE.Material): void => {
          ...
          const map = (material as THREE.Material & { map?: THREE.Texture })
            .map;
          if (map) textures.add(map);
        };
        if (Array.isArray(mesh.material))
          mesh.material.forEach(collectMaterial);
...
function makeMaterials(p: Palette): Record<string, THREE.MeshStandardMaterial> {
  ...
  return {
    ground: material(p.ground),
    route: material(p.route, 0.72, p.route),
    routeSpur: new THREE.MeshStandardMaterial({
      color: p.route,
      emissive: p.route,
    ...
    structure: material(p.structure),
    shadow: material(p.shadow),
    trim: material(p.trim, 0.55),
    glow: material(p.glow, 0.48, p.glow),
    warning: material(p.warning, 0.45, p.warning),
    leaf: material(p.leaf, 0.9),
    white: material(0xe3edff, 0.65),
    red: material(0xd95050, 0.66),
    black: material(0x10151b, 0.75),
  };
}
...
function addRouteRibbon(
  ...
  for (const [index, route] of layout.routes.entries()) {
    ...
    const ribbon = new THREE.Mesh(
      geometry,
      index === 0 ? primaryMat : secondaryMat
    );
  ...
  for (const [index, shape] of layout.collision.entries()) {
    ...
    if (layout.floor === 4) {
      ...
      const top = new THREE.Mesh(
        new THREE.BoxGeometry(width * 1.05, 0.14, depth * 1.05),
        mats.trim
      );
      ...
      const rail = new THREE.Mesh(
        new THREE.BoxGeometry(
          horizontal ? length : 0.42,
          0.55,
          horizontal ? 0.42 : length
        ),
        mats.structure
      );
      ...
      for (let i = 0; i < pierCount; i += 1) {
        ...
        const pier = new THREE.Mesh(
          new THREE.CylinderGeometry(0.42, 0.62, height, 6),
          mats.structure
        );
        pier.position.set(
          horizontal ? along : 0,
      ...
      const railTop = new THREE.Mesh(
        new THREE.BoxGeometry(
          horizontal ? length : 0.6,
          0.12,
          horizontal ? 0.6 : length
        ),
        mats.trim
      );
...
function addLandmarks(
  ...
  for (const landmark of layout.landmarks) {
    ...
    else if (landmark.role === "offer")
      addOfferLandmark(group, layout, mats, p);
    else if (landmark.role === "discovery")
      addDiscoveryLandmark(group, layout, mats, p);
    else addOrientationLandmark(group, layout, mats);
    root.add(group);
  }
}

function addOrientationLandmark(
  group: THREE.Group,
  layout: MiddleFloorLayout,
  mats: Record<string, THREE.MeshStandardMaterial>
): void {
...
function addDiscoveryLandmark(
  ...
  if (layout.floor === 4) {
    const panel = new THREE.Mesh(
      new THREE.BoxGeometry(1.6, 1.15, 0.24),
      mats.shadow
    );
    ...
    const halo = new THREE.Mesh(
      new THREE.TorusGeometry(0.72, 0.045, 5, 12),
      mats.trim
    );
  ...
  for (let i = 0; i < 3; i += 1) {
    ...
    if (layout.floor === 4) {
      const frame = new THREE.Mesh(
        new THREE.BoxGeometry(0.8, 1.35, 0.18),
        mats.structure
      );
      frame.position.y = 0.68;
      marker.add(frame);
      const inset = new THREE.Mesh(
        new THREE.BoxGeometry(0.26, 0.42, 0.08),
        colors[i]!
      );
      ...
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(0.46, 0.035, 5, 10),
        mats.trim
      );
      ring.position.y = 1.1;
      marker.add(ring);
  ...
  if (layout.floor === 4) {
    ...
    for (const x of [-2.2, 2.2]) {
      ...
      const light = new THREE.Mesh(
        new THREE.BoxGeometry(0.12, 2.65, 0.08),
        mats.warning
      );
    ...
    const lintel = new THREE.Mesh(
      new THREE.BoxGeometry(5, 0.48, 0.6),
      mats.trim
    );
    lintel.position.y = 3.2;
    group.add(lintel);
    const threshold = new THREE.Mesh(
      new THREE.BoxGeometry(3.7, 0.06, 1.7),
      mats.glow
    );
    ...
    const frame = new THREE.Mesh(
      new THREE.TorusGeometry(2.05, 0.22, 6, 14),
      mats.trim
    );
    ...
    for (const x of [-2.2, 2.2]) {
      const pier = new THREE.Mesh(
        new THREE.CylinderGeometry(0.28, 0.58, 3.5, 6),
        mats.structure
      );
    ...
    const arch = new THREE.Mesh(
      new THREE.TorusGeometry(2.2, 0.28, 6, 12, Math.PI),
      mats.trim
    );
    arch.position.set(0, 1.72, 0);
    group.add(arch);
    const strip = new THREE.Mesh(
      new THREE.BoxGeometry(3.3, 0.06, 1.8),
      mats.glow
    );
...
function addEraSetDressing(
  ...
  if (layout.floor === 4) {
    ...
    const shelfFaceMaterials = Array.from({ length: 4 }, (_, variant) =>
      makeArchiveShelfMaterial(variant)
    );
    ...
    for (let side = -1; side <= 1; side += 2) {
      for (let i = 0; i < 4; i += 1) {
        ...
        addArchiveShelfFront(
          shelf,
          shelfFaceGeometry,
          shelfFaceMaterials[i % shelfFaceMaterials.length]!,
          4.2
        );
    ...
    const dial = new THREE.Mesh(
      new THREE.TorusGeometry(3.2, 0.08, 5, 18),
      mats.trim
    );
    dial.position.set(-15, 0.08, 4);
    dial.rotation.x = Math.PI / 2;
    root.add(dial);
    const innerDial = new THREE.Mesh(
      new THREE.TorusGeometry(1.8, 0.06, 5, 14),
      mats.glow
    );
    ...
    for (let i = 0; i < 9; i += 1) {
      const shard = new THREE.Mesh(
        new THREE.OctahedronGeometry(0.32 + (i % 3) * 0.12, 0),
        i % 2 ? mats.trim : mats.glow
      );
      shard.position.set(
        -18 + ((i * 9) % 36),
        3.2 + (i % 3),
...
function addObservatoryGroundMotif(
  root: THREE.Group,
  layout: MiddleFloorLayout
): void {
  const candidates = [
    {
      x: layout.bounds.minX + 7,
      z: (layout.bounds.minZ + layout.bounds.maxZ) / 2 + 4,
    },
    {
      x: layout.bounds.maxX - 7,
      z: (layout.bounds.minZ + layout.bounds.maxZ) / 2 + 4,
    },
  ];
  const point = candidates.find(
    (candidate) =>
      !isNearMiddleRoute(candidate, layout, 2.8) &&
      !isInsideCollision(candidate, layout, 2.8)
  );
  if (!point) return;
  ...
  if (!context)
    throw new Error(
      "Canvas 2D context unavailable for Floor 6 observatory motif"
    );
...
function makeArchiveShelfMaterial(variant: number): THREE.MeshBasicMaterial {
  ...
  if (!ctx)
    throw new Error("Canvas 2D context unavailable for Floor 5 archive art");
  const palettes = [
...
function makeArchiveSilhouetteMaterial(): THREE.MeshBasicMaterial {
  ...
  if (!ctx)
    throw new Error(
      "Canvas 2D context unavailable for Floor 5 archive silhouette"
    );
...
function addArchiveBanner(
  ...
  const pole = new THREE.Mesh(
    new THREE.BoxGeometry(width + 0.28, 0.12, 0.12),
    mats.trim
  );
  ...
  const mark = new THREE.Mesh(
    new THREE.BoxGeometry(0.32, height * 0.42, 0.06),
    mats.warning
  );
  mark.position.set(x, y + 0.04, z + 0.095);
  group.add(mark);
  const hem = new THREE.Mesh(
    new THREE.BoxGeometry(width * 0.72, 0.1, 0.07),
    mats.trim
  );
...
function addArchiveLamp(
  ...
  const mount = new THREE.Mesh(
    new THREE.BoxGeometry(0.3, 0.34, 0.16),
    mats.trim
  );
  mount.position.set(x, y + 0.5, z);
  group.add(mount);
  const hood = new THREE.Mesh(
    new THREE.BoxGeometry(0.82, 0.18, 0.4),
    mats.structure
  );
  hood.position.set(x, y + 0.22, z + 0.1);
  group.add(hood);
  const light = new THREE.Mesh(
    new THREE.BoxGeometry(0.48, 0.42, 0.24),
    mats.warning
  );
...
function addArchiveShelfSilhouettes(
  root: THREE.Group,
  layout: MiddleFloorLayout
): void {
  const width = layout.bounds.maxX - layout.bounds.minX;
...
function addEchoPlinths(
  ...
  const { minX, maxX, minZ, maxZ } = layout.bounds;
  const candidates = [
    { x: minX + 2.4, z: minZ + 2.4 },
    { x: maxX - 2.4, z: minZ + 2.4 },
    { x: minX + 2.4, z: maxZ - 2.4 },
    { x: maxX - 2.4, z: maxZ - 2.4 },
  ];
  let index = 0;
  for (const point of candidates) {
    if (
      ...
      continue;
    const overlapsEnemy = layout.encounter.enemies.some(
      (enemy) =>
        Math.hypot(point.x - enemy.spawn.x, point.z - enemy.spawn.z) < 2.4
    );
    if (
      overlapsEnemy ||
      isNearMiddleRoute(point, layout, 2.4) ||
      isInsideCollision(point, layout, 1.25)
    )
      continue;
...
function makeEchoPlinth(
  mats: Record<string, THREE.MeshStandardMaterial>
): THREE.Group {
  const plinth = new THREE.Group();
  plinth.name = "echo-plinth-form";
  const base = new THREE.Mesh(
    new THREE.BoxGeometry(1.3, 0.22, 1.05),
    mats.structure
  );
  base.position.y = 0.12;
  plinth.add(base);
  const inset = new THREE.Mesh(
    new THREE.BoxGeometry(0.96, 0.1, 0.76),
    mats.shadow
  );
...
function isInsideCollision(
  point: FloorPoint,
  layout: MiddleFloorLayout,
  clearance: number
): boolean {
  return layout.collision.some(
    (shape) =>
      shape.kind === "rect" &&
...
function addEncounterRecoveryZones(
  root: THREE.Group,
  layout: MiddleFloorLayout,
  mats: Record<string, THREE.MeshStandardMaterial>
): void {
  ...
  for (const [index, zone] of layout.encounter.arena.recoveryZones.entries()) {
    ...
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(zone.radius, 0.045, 5, 16, Math.PI * 1.6),
      mats.route
    );
    ring.rotation.x = Math.PI / 2;
    ...
    marker.add(ring);
    for (let i = 0; i < 3; i += 1) {
      const markerTile = new THREE.Mesh(
        new THREE.BoxGeometry(0.28, 0.035, 0.28),
        mats.trim
      );
      const angle = (i / 3) * Math.PI * 2 + 0.3;
      markerTile.position.set(
        Math.cos(angle) * zone.radius * 0.72,
        0.03,
        Math.sin(angle) * zone.radius * 0.72
      );
...
function addVaultSentinel(
  ...
  for (let i = 0; i < 4; i += 1) {
    ...
    addLimb(
      group,
      { x: side * 0.54, z: fore * 0.28 },
      { x: side * 1.04, z: fore * 0.68 },
      mats.black,
      mats.warning,
      0.12
    );
  }
  const chargeBlade = new THREE.Mesh(
...
function addLibraryEnemy(
  ...
  if (enemy.role === "charger") {
    const shell = new THREE.Mesh(
      new THREE.SphereGeometry(0.65, 8, 5),
      mats.leaf
    );
    ...
    const cover = new THREE.Mesh(
      new THREE.ConeGeometry(0.62, 0.75, 5),
      mats.trim
    );
    ...
    const halo = new THREE.Mesh(
      new THREE.TorusGeometry(0.8, 0.045, 4, 12),
      mats.glow
    );
    halo.position.y = 1.2;
...
function addObservatoryEnemy(
  ...
  if (enemy.role === "charger") {
    ...
    const brow = new THREE.Mesh(
      new THREE.ConeGeometry(0.44, 0.72, 5),
      mats.warning
    );
    ...
    for (let i = 0; i < 4; i += 1) {
      ...
      addLimb(
        group,
        { x: side * 0.5, z: fore * 0.3 },
        { x: side * 0.88, z: fore * 0.66 },
        mats.shadow,
        mats.trim,
        0.14
      );
    ...
    const crystal = new THREE.Mesh(
      new THREE.OctahedronGeometry(0.73, 0),
      mats.glow
    );
    ...
    const rings = [0.85, 1.12].map((radius, index) => {
      const ring = new THREE.Mesh(
        new THREE.TorusGeometry(radius, 0.05, 5, 12),
        index ? mats.trim : mats.warning
      );
      ring.position.y = 1.2;
    ...
    for (const x of [-1.05, 1.05]) {
      const fin = new THREE.Mesh(
        new THREE.ConeGeometry(0.19, 0.68, 4),
        mats.structure
      );
...
function addEffectSockets(
  root: THREE.Group,
  layout: MiddleFloorLayout,
  mats: Record<string, THREE.MeshStandardMaterial>,
  telegraphAimers: Map<string, (target: FloorPoint) => void>
): void {
  for (const cue of layout.effects) {
    ...
    if (cue.trigger === "enemy-telegraph") {
      const enemy = layout.encounter.enemies.find(
        (candidate) => candidate.id === cue.enemyId
      );
      if (enemy)
        telegraphAimers.set(enemy.id, addTelegraph(socket, enemy, mats));
...
function addTelegraph(
  ...
  if (enemy.role === "ranged") {
    ...
    const inner = new THREE.Mesh(
      new THREE.RingGeometry(0.95, 1.08, 4),
      mats.glow
    );
    ...
    const lane = new THREE.Mesh(
      new THREE.PlaneGeometry(0.95, length),
      mats.warning
    );
...
function addOutcomeSocket(
  ...
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(1.5, 0.075, 5, 14),
    mats.glow
  );
...
function addLeafCluster(
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    leaf.position.set(
      centerX + (((i * 7) % 11) / 10 - 0.5) * width,
      y + (i % 3) * 0.13,
      centerZ + (((i * 5) % 9) / 8 - 0.5) * depth
    );
    leaf.rotation.set(
      (i % 2) * 0.18,
      (i * 0.73) % (Math.PI * 2),
...
function addPageSprite(
  ...
  const page = new THREE.Mesh(
    new THREE.ShapeGeometry(makePageShape(width, height)),
    mat
  );
  ...
  const band = new THREE.Mesh(
    new THREE.PlaneGeometry(width * 0.62, 0.055),
    mat
  );
...
function addPrism(
  ...
  const lower = new THREE.Mesh(
    new THREE.TetrahedronGeometry(scale * 0.42, 0),
    mat
  );
...
function addLimb(
  ...
  const foot = new THREE.Mesh(
    new THREE.ConeGeometry(radius * 1.2, 0.3, 4),
    footMaterial
  );
...
function addCylinderBetween(
  ...
  const mesh = new THREE.Mesh(
    new THREE.CylinderGeometry(radius * 0.7, radius, delta.length(), segments),
    mat
  );
