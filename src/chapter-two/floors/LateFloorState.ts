import { FLOOR_7_TOWN, type LateFloorPoint, type TownRoute } from "./late-floor-7-town";
import { FLOOR_8_FINALE } from "./late-floor-8-finale";
import type { Guide } from "../rooms";

export type LateFloorId = 7 | 8;
export type LateFloorStatus = "exploring" | "ready-to-choose" | "route-selected" | "core-approach" | "complete";
export type CollectorPhase = "telegraphing" | "sweeping" | "cooldown" | "stunned";
export type LateFloorInteraction = "dw-light" | "dw-shadow" | "dw-ambition" | "exit-boulevard" | "exit-alleys" | "exit-core" | "healing-core";
...
export class LateFloorState {
    ...
    return FLOOR_7_TOWN.encounters
      ...
      .map((encounter, index) => {
        ...
        return {
          ...
          phase: this._collectorStunSeconds[index] > 0
            ? "stunned"
            : phaseTime < 0.9
              ? "telegraphing"
              : phaseTime < 2.4
  ...
  public update(deltaSeconds: number, axes: LateFloorAxes): void {
    if (this._status === "complete" || !Number.isFinite(deltaSeconds) || deltaSeconds <= 0) return;
    ...
    if (this.floor === 7) {
      ...
      const core = FLOOR_8_FINALE.landmarks.find((landmark) => landmark.id === "healing-core");
  ...
  public hit(targetId?: string): boolean {
    ...
    const candidates = this.collectors
      .filter((collector) => collector.phase !== "stunned" && (targetId === undefined || collector.id === targetId))
      .map((collector) => ({ collector, range: distance(this.player, collector.position) }))
      .filter(({ range }) => range <= 2.25)
      .sort((a, b) => a.range - b.range);
    const target = candidates[0]?.collector;
    if (!target) return false;
    const index = target.id === "collector-sweeper" ? 0 : 1;
    this._collectorHealth[index] = Math.max(0, this._collectorHealth[index] - 1);
  ...
  private _nearestDreamweaver(): string | null {
    const nearest = FLOOR_7_TOWN.landmarks
      ...
      .map((landmark) => ({ landmark, range: distance(this.player, landmark.position) }))
  ...
  private _gather(id: string): boolean {
    const guideById: Record<string, Guide> = { "dw-light": "Light", "dw-shadow": "Shadow", "dw-ambition": "Ambition" };
    const guide = guideById[id];
    const landmark = FLOOR_7_TOWN.landmarks.find((item) => item.id === id);
    if (!guide || !landmark || distance(this.player, landmark.position) > 3 || this.gathered.has(guide)) return false;
    this.gathered.add(guide);
    if (this.gathered.size === 3) this._status = "ready-to-choose";
    return true;
  }

  private _move(layout: typeof FLOOR_7_TOWN | typeof FLOOR_8_FINALE, dt: number, axes: LateFloorAxes): void {
    ...
    const nextX = clamp(this.player.x + (axes.x / magnitude) * step, bounds.x - bounds.width / 2, bounds.x + bounds.width / 2);
    const nextZ = clamp(this.player.z + (axes.z / magnitude) * step, bounds.z - bounds.depth / 2, bounds.z + bounds.depth / 2);
    const radius = FLOOR_7_TOWN.playerCollision.radius;
    const tryAxis = (x: number, z: number): boolean => layout.collision.every((box) => !circleOverlapsBox(x, z, radius, box));
  ...
  private _updateCollectors(dt: number): void {
    ...
    for (let i = 0; i < this._collectorStunSeconds.length; i++) {
      ...
      this._collectorStunSeconds[i] = Math.max(0, this._collectorStunSeconds[i] - dt);
      if (this._collectorStunSeconds[i] === 0) this._collectorHealth[i] = 2;
    }
    const activeCollectors = this.collectors.filter((collector) => collector.phase !== "stunned");
    const exposed = activeCollectors.some((collector) =>
      collector.phase !== "cooldown" && distance(this.player, collector.position) <= 3.2
    );
    const contact = activeCollectors.some((collector) =>
      collector.phase === "sweeping" && distance(this.player, collector.position) <= 1.25
    );
    this._collectorPressure = clamp(this._collectorPressure + (exposed ? dt : -dt * 0.18) + (contact ? dt * 2 : 0), 0, 3);
  ...
  private _tryEnterFloor8(): void {
    const route = FLOOR_7_TOWN.routes.find((candidate) => candidate.id === this._selectedRoute);
    const exit = route && FLOOR_7_TOWN.exits.find((candidate) => candidate.id === route.exitId);
  ...
  private _overlapsExit(bounds: { x: number; z: number; width: number; depth: number }): boolean {
    const radius = FLOOR_7_TOWN.playerCollision.radius;
    return Math.abs(this.player.x - bounds.x) <= bounds.width / 2 + radius &&
...
function circleOverlapsBox(x: number, z: number, radius: number, box: { x: number; z: number; width: number; depth: number }): boolean {
