...
export interface EncounterRecoveryZone {
  center: FloorPoint;
  radius: number;
}
export interface MiddleFloorEncounter {
  ...
  arena: {
    ...
    width: number;
    depth: number;
    recoveryZones: readonly EncounterRecoveryZone[];
  };
...
export interface MiddleFloorDefinition extends Omit<
  MiddleFloorLayout,
  "variation"
> {
...
export function deterministicVariantIndex(
  seed: number,
  floor: MiddleFloorNumber,
  optionCount: number
): number {
...
export function resolveMiddleFloor(
  definition: MiddleFloorDefinition,
  seed: number
): MiddleFloorLayout {
  const index = deterministicVariantIndex(
    seed,
    definition.floor,
    definition.variationOptions.length
  );
  const selected = definition.variationOptions[index] ?? {
    surface: "stable-grid",
    accent: "warm-white",
  };
