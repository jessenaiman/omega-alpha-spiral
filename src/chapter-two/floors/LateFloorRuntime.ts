...
import {
  ...
  type LateFloorAxes,
  type LateFloorId,
  type LateFloorInteraction,
  type LateFloorStatus,
...
export type LateFloorRecruitChoices = Readonly<
  Partial<Record<4 | 5 | 6, Guide>>
>;
...
export class LateFloorRuntime {
  ...
  private readonly _lastCollectorPhases = new Map<
    string,
    CollectorSnapshot["phase"]
  >();
  ...
  public constructor(recruitChoices: LateFloorRecruitChoices = {}) {
    ...
    for (const follower of this._partyFollowers)
      this._partyGroup.add(follower.root);
    ...
    this._floaterBase =
      this._townArt.group.getObjectByName("floating-sign")?.position.clone() ??
  ...
  public update(
    ...
    if (this._state.memoryRecycles !== this._seenRecycleCount) {
      this._seenRecycleCount = this._state.memoryRecycles;
      this._recycleTimer =
        FLOOR_7_TOWN.effects.find((effect) => effect.cue === "memory-recycle")
          ?.durationSeconds ?? 1.1;
    ...
    if (this._state.status === "complete" && this._previousPhase !== "complete")
      this._audio.cue(523.25, 0.42, 0.025);
    this._previousPhase = this._state.status;
  }
  ...
  public hit(): boolean {
    ...
    if (changed) {
      const stunned = this._state.collectors.some(
  ...
  private _updatePartyFollowers(dt: number, reducedMotion: boolean): void {
    ...
    this._partyFollowers.forEach((follower, index) => {
      ...
      follower.root.position.set(
        ...
        reducedMotion
          ? 0
          : Math.sin(this._followerPhase * 3.1 + index * 1.7) * 0.035,
        playerZ + backZ * offset.back + sideZ * offset.side
      );
      follower.root.rotation.y = Math.atan2(
        this._trailDirection.x,
        this._trailDirection.y
      );
    });
  }
  ...
  private _animateTown(dt: number, reducedMotion: boolean): void {
    ...
    if (sign) {
      sign.position.y =
        this._floaterBase.y +
        (reducedMotion ? 0 : Math.sin(this._elapsed * 1.1) * 0.11);
      sign.rotation.y = reducedMotion
        ? 0
        : Math.sin(this._elapsed * 0.7) * 0.035;
    }

    const terminalScreen = this._townArt.group
      .getObjectByName("local-terminal")
      ?.getObjectByName("terminal-screen");
    if (terminalScreen) {
      const activeRestart = this._recycleTimer > 0;
      const blink =
        activeRestart && !reducedMotion
          ? 0.65 + 0.35 * Math.sin(this._elapsed * 26)
          : 1;
      terminalScreen.scale.y = blink;
      setEmissive(
        terminalScreen,
        activeRestart ? (reducedMotion ? 0.18 : 0.15 + blink * 0.5) : 0.05
      );
    }

    const memory = this._townArt.group.getObjectByName("memory-well");
    if (memory) {
      const fragments = memory.children.filter(
        (child) => child.name === "memory-fragment"
      );
      fragments.forEach((fragment, index) => {
        if (this._recycleTimer <= 0) {
        ...
        const progress =
          1 -
          this._recycleTimer /
            (FLOOR_7_TOWN.effects.find(
              (effect) => effect.cue === "memory-recycle"
            )?.durationSeconds ?? 1.1);
        const dissolveIndex = Math.floor(progress * (fragments.length + 1));
        fragment.visible = reducedMotion ? true : index >= dissolveIndex;
        if (!reducedMotion && fragment.visible)
          fragment.rotation.y += dt * (index % 2 ? 1 : -1);
      });
    }
    ...
    for (const collector of this._state.collectors) {
      ...
      actor.rotation.y =
        collector.phase === "stunned" || reducedMotion
          ? 0
          : this._elapsed * 0.7;
      actor.rotation.z = collector.phase === "stunned" ? 0.08 : 0;
      const fan =
        actor.getObjectByName("forward-scan-fan") ??
        actor.getObjectByName("cyan-leading-ring");
      if (fan)
        fan.visible =
          collector.phase === "telegraphing" || collector.phase === "sweeping";
  ...
  private _animateFinale(reducedMotion: boolean): void {
    ...
    const convergence = Math.max(
      0,
      Math.min(1, (20 - this._state.player.z) / 3)
    );
    const arch = threshold?.getObjectByName("threshold-light-arch");
    if (arch) {
      const pulse = reducedMotion
        ? 1
        : 1 + Math.sin(this._elapsed * 2.4) * 0.035 * convergence;
      arch.scale.setScalar(pulse);
      setEmissive(
        arch,
        0.12 +
          convergence *
            (reducedMotion ? 0.32 : 0.25 + Math.sin(this._elapsed * 2.4) * 0.12)
      );
    }
    ...
    const distanceToCore = Math.hypot(
      this._state.player.x,
      this._state.player.z + 15
    );
    const proximity = Math.max(0, Math.min(1, (9 - distanceToCore) / 7));
    const pulse = reducedMotion
      ? 1
      : 1 + Math.sin(this._elapsed * 2.8) * 0.07 * proximity;
    if (core) {
      ...
      setEmissive(
        ...
        0.45 +
          proximity *
            (reducedMotion ? 0.3 : 0.35 + Math.sin(this._elapsed * 2.8) * 0.2)
      );
    }
  ...
  private _playCollectorPhaseCues(): void {
    for (const collector of this._state.collectors) {
      ...
      if (collector.phase === "sweeping" && last !== "sweeping")
        this._audio.cue(92, 0.1, 0.012);
...
function createPartyFollowers(
  recruitChoices: LateFloorRecruitChoices
): readonly PartyFollower[] {
  ...
  const forms: Array<"scout" | "scribe" | "vanguard"> = [
    "scout",
    "scribe",
    "vanguard",
  ];
  return forms.map((form, index) => {
    ...
    const accentColor = guide ? alignmentPalette[guide] : defaultPalette[index];
    ...
    capsule(
      form === "vanguard" ? 0.34 : 0.27,
      form === "scout" ? 0.53 : 0.66,
      body,
      "silhouette-torso",
      0.66
    );
    ...
    if (form === "scout") {
      const hood = mesh(
        new THREE.ConeGeometry(0.32, 0.52, 5),
        accent,
        "scout-hood",
        1.25
      );
      ...
      const cloak = mesh(
        new THREE.ConeGeometry(0.45, 0.82, 5, 1, true),
        body,
        "scout-cloak",
        0.56
      );
      ...
      const halo = mesh(
        new THREE.TorusGeometry(0.31, 0.045, 5, 12),
        accent,
        "scribe-halo",
        1.5
      );
    ...
    const feet = mesh(
      new THREE.TorusGeometry(0.29, 0.025, 4, 14),
      accent,
      "grounding-ring",
      0.08
    );
