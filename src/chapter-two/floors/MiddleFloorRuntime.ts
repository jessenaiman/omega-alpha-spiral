...
import { createMiddleFloorArtKit, type MiddleFloorArtResult } from "./MiddleFloorArtKit";
import type {
  FloorEffectCue,
  FloorLandmark,
  FloorPoint,
...
const RECRUIT_CANDIDATES: readonly RecruitGuide[] = ["Light", "Shadow", "Ambition"];
...
export class MiddleFloorRuntime {
  ...
  private readonly _recruitChoices: Partial<Record<MiddleFloorNumber, RecruitGuide>> = {};
  ...
  public get playerPosition(): FloorPoint {
    return point(this._combat?.phase === "active" ? this._combat.player.position : this._playerPosition);
  }
  ...
  public get recruitChoices(): Readonly<Partial<Record<MiddleFloorNumber, RecruitGuide>>> {
    return { ...this._recruitChoices };
  }

  public get status(): MiddleFloorRuntimeStatus | null {
    if (!this._layout) return null;
    const offer = this._layout.landmarks.find((landmark) => landmark.role === "offer") ?? null;
    const position = this.playerPosition;
    const nearOffer = Boolean(offer && distanceBetween(position, offer.position) <= offer.reach);
    ...
    const currentTelegraph = this._combat?.phase === "active"
      ? this._combat.enemies.find((enemy) => enemy.telegraph)?.telegraph ?? null
      : null;
    ...
    const objective = this._combat?.resolution && !recruitChoice
      ? this._combat.resolution.outcome === "fallen"
  ...
  public enterFloor(floor: MiddleFloorNumber, seed: number = this._defaultSeed): void {
    if (this._disposed) return;
    this._clearFloorContent();
    const safeSeed = Number.isFinite(seed) ? Math.trunc(seed) : this._defaultSeed;
  ...
  public update(deltaMs: number, movement: MiddleFloorMovement = { x: 0, z: 0 }): void {
    if (
      this._disposed ||
      !this._layout ||
  ...
  public hit(): boolean {
    if (this._disposed || !this._combat || this._combat.phase !== "active") return false;
  ...
  public run(direction: MiddleFloorMovement = { x: 0, z: 0 }): boolean {
    if (this._disposed || !this._layout || this._transitionSignal !== null) return false;
    if (this._combat?.phase === "active") {
      ...
      const events = escaped.events.map((event): CombatEvent =>
        event.type === "player-evaded" ? { ...event, to: point(destination) } : event
      );
      this._applyCombatState({
        ...escaped,
  ...
  public chooseRecruit(guide: RecruitGuide): boolean {
    const offer = this._layout?.landmarks.find((landmark) => landmark.role === "offer");
    if (!offer || !RECRUIT_CANDIDATES.includes(guide)) return false;
    if (distanceBetween(this.playerPosition, offer.position) > offer.reach) return false;
  ...
  private _moveOnFloor(deltaMs: number, movement: MiddleFloorMovement): void {
    ...
    this._playerPosition = point(this._slideMove(this._playerPosition, requested, false));
  }
  ...
  private _slideMove(start: FloorPoint, destination: FloorPoint, inArena: boolean): FloorPoint {
  ...
  private _moveSegment(start: FloorPoint, destination: FloorPoint, inArena: boolean): FloorPoint {
  ...
  private _canOccupy(position: FloorPoint, inArena: boolean): boolean {
    ...
    if (inArena && this._combat && !insideEncounterArena(position, this._combat)) return false;
    return layout.collision.every((shape) => {
      if (shape.kind === "rect") {
        return (
          Math.abs(position.x - shape.center.x) >= shape.width / 2 + PLAYER_RADIUS ||
          Math.abs(position.z - shape.center.z) >= shape.depth / 2 + PLAYER_RADIUS
        );
      }
      return distanceBetween(position, shape.center) >= shape.radius + PLAYER_RADIUS;
    });
  }

  private _maybeEnterEncounter(): void {
    const layout = this._layout;
    if (!layout || this._combat || !insideAuthoredArena(this._playerPosition, layout)) return;
  ...
  private _handleCombatEvent(event: CombatEvent): void {
    ...
    if (event.type === "telegraph-started") {
      ...
      this._activateMatchingCues("encounter-resolved", { outcome: event.outcome });
      const resolution = this._combat?.resolution;
      if (resolution && !this._recordedEncounters.has(resolution.echo.encounterId)) {
  ...
  private _updateLandmarksAndExit(): void {
    ...
    for (const landmark of layout.landmarks) {
      const inside = distanceBetween(position, landmark.position) <= landmark.reach;
      ...
      if (inside && !this._discoveredLandmarks.has(landmark.id)) {
        ...
        this._activateMatchingCues("discover-landmark", { landmarkId: landmark.id });
      }
      const hasRecruitChoice = Boolean(this._recruitChoices[layout.floor]);
      if (landmark.role === "exit" && this._combat?.resolution && hasRecruitChoice && inside) {
        if (!wasInside) this._activateMatchingCues("reach-exit", { landmarkId: landmark.id });
        if (!this._exitCrossed && distanceBetween(position, landmark.position) <= EXIT_CROSS_RADIUS) {
  ...
  private _activateMatchingCues(
    ...
    match: { readonly landmarkId?: string; readonly enemyId?: string; readonly outcome?: CombatOutcome }
  ): void {
    for (const cue of this._layout?.effects ?? []) {
      if (cue.trigger !== trigger) continue;
      if (cue.encounterId && cue.encounterId !== this._layout?.encounter.id) continue;
  ...
  private _activateCue(cue: FloorEffectCue, outcome?: CombatOutcome): void {
    ...
    this._cueReadyAt.set(cue.id, this._clockMs + Math.max(250, cue.visual.durationMs));
    this._playAudio(cue);
  }
  ...
  private _syncTelegraphSockets(): void {
    ...
    for (const enemy of combat.enemies) {
      ...
      const targetSocket = this.group.getObjectByName(`telegraph-target:${enemy.id}`);
      if (telegraph) {
  ...
  private _createFallbackTelegraphs(): void {
    ...
    for (const enemy of layout.encounter.enemies) {
      ...
      if (enemy.role === "ranged") {
        ...
        const ring = new THREE.Mesh(new THREE.RingGeometry(0.72, 0.96, 4), warning);
        ...
        const arc = new THREE.Mesh(new THREE.TorusGeometry(1.75, 0.055, 4, 12), warning);
        ...
        const lane = new THREE.Mesh(new THREE.PlaneGeometry(1.1, length), warning);
        lane.rotation.x = -Math.PI / 2;
        lane.position.set(0, 0.03, -length / 2);
        root.add(lane);
        const markerArrow = new THREE.Mesh(new THREE.ConeGeometry(0.26, 0.5, 3), warning);
  ...
  private _aimFallback(
    ...
    if (fallback.shape === "charge-lane") {
      ...
      if (Math.hypot(dx, dz) > 0.001) fallback.root.rotation.y = Math.atan2(-dx, -dz);
    }
  }
...
function createLayout(floor: MiddleFloorNumber, seed: number): MiddleFloorLayout {
...
function insideEncounterArena(position: FloorPoint, combat: CombatEncounterState): boolean {
...
function insideAuthoredArena(position: FloorPoint, layout: MiddleFloorLayout): boolean {
