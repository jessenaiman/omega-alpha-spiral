...
export type CombatEvent =
  ...
  | {
      readonly type: "player-evaded";
      readonly from: FloorPoint;
      readonly to: FloorPoint;
    }
  | {
      readonly type: "player-recovered";
...
export function createCombatEncounter(
  ...
  if (encounter.enemies.length < 1 || encounter.enemies.length > 2) {
    throw new Error(
      `Encounter ${encounter.id} must contain one or two enemies.`
    );
  }
  ...
  const tuning: Required<CombatEncounterOptions> = {
    ...
    evadeCooldownMs: positive(
      options.evadeCooldownMs,
      DEFAULTS.evadeCooldownMs
    ),
  };
...
export function evadeCombatPlayer(
  ...
  const vector =
    escapingThreat || (preferred.x === 0 && preferred.z === 0)
      ? away
      : normalize({
...
function advanceFrame(
  ...
  const playerPosition = clampToArena(
    {
      x:
        ...
        (direction.x * state.tuning.moveSpeed * deltaMs) / 1000,
      z:
        state.player.position.z +
        (direction.z * state.tuning.moveSpeed * deltaMs) / 1000,
    },
    state.arena
  );
  let player: CombatPlayerState = {
    ...
    hitCooldownRemainingMs: Math.max(
      0,
      state.player.hitCooldownRemainingMs - deltaMs
    ),
    evadeCooldownRemainingMs: Math.max(
      0,
      state.player.evadeCooldownRemainingMs - deltaMs
    ),
  };
  ...
  const enemies = state.enemies.map((enemy) => {
    ...
    const cooldownRemainingMs = Math.max(
      0,
      enemy.cooldownRemainingMs - deltaMs
    );
    if (
      cooldownRemainingMs === 0 &&
...
function attackHits(telegraph: CombatTelegraph, player: FloorPoint): boolean {
  ...
  return (
    distanceToSegment(player, telegraph.origin, telegraph.target) <=
    CHARGE_HALF_WIDTH
  );
}
...
function clampToArena(
  ...
  return {
    x: Math.max(
      arena.center.x - halfWidth,
      Math.min(arena.center.x + halfWidth, position.x)
    ),
    z: Math.max(
      arena.center.z - halfDepth,
      Math.min(arena.center.z + halfDepth, position.z)
    ),
  };
}
...
function distanceToSegment(
  ...
  const projection = Math.max(
    ...
    Math.min(
      1,
      ((pointValue.x - start.x) * dx + (pointValue.z - start.z) * dz) /
        lengthSquared
    )
  );
