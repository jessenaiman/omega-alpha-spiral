...
function makeTownMaterials(
  owner: ReturnType<typeof ownResources>
): TownMaterials {
  const standard = (
    color: number,
    roughness = 0.88,
  ...
  return {
    ...
    paving: owner.material(
      new THREE.MeshStandardMaterial({
        map: createPavingTexture(owner),
        color: 0xffffff,
        roughness: 0.96,
        metalness: 0,
      })
    ),
    route: owner.material(
    ...
    windowGlow: owner.material(
      new THREE.MeshBasicMaterial({ color: 0xffc66c })
    ),
...
function addPavedGround(
  ...
  border.setFromPoints([
    new THREE.Vector3(x0, 0.006, z0),
    new THREE.Vector3(x1, 0.006, z0),
    new THREE.Vector3(x1, 0.006, z0),
    new THREE.Vector3(x1, 0.006, z1),
    new THREE.Vector3(x1, 0.006, z1),
    new THREE.Vector3(x0, 0.006, z1),
    new THREE.Vector3(x0, 0.006, z1),
    new THREE.Vector3(x0, 0.006, z0),
  ]);
...
function createPavingTexture(
  ...
  if (!context)
    throw new Error("Town paving texture requires a 2D canvas context.");
  ...
  for (let row = 0; row < 16; row += 1) {
    ...
    for (let column = 0; column < 8; column += 1) {
      ...
      context.stroke();
      context.fillStyle =
        (row + column) % 3 ? "rgba(170,190,219,.08)" : "rgba(20,25,40,.12)";
  ...
  context.save();
  context.beginPath();
  context.rect(splitA, 0, splitB - splitA, canvas.height);
  context.clip();
    ...
    for (let column = -1; column < 10; column += 1) {
      ...
      context.fillStyle =
        (row + column) % 4 === 0
          ...
          : "rgba(206,171,138,.07)";
      context.fill();
      context.strokeStyle = "rgba(218,193,157,.12)";
      context.lineWidth = 1;
      context.stroke();
    }
  }
  ...
  context.save();
  context.beginPath();
  context.rect(splitB, 0, canvas.width - splitB, canvas.height);
  context.clip();
  for (let row = -1; row < 18; row += 1) {
    for (let column = -1; column < 10; column += 1) {
      ...
      context.fill();
      context.beginPath();
      context.moveTo(x + 30, y + 5);
      context.lineTo(x + 30, y + 30);
      context.lineTo(x + 8, y + 30);
      context.closePath();
      context.fillStyle =
        (row + column) % 3 ? "rgba(20,28,44,.12)" : "rgba(164,181,198,.09)";
      context.fill();
    }
  }
  context.restore();
...
function addPlazaMosaic(
  ...
  const disk = mesh(
    owner,
    new THREE.CircleGeometry(7.7, 40),
    mats.stone,
    "town-plaza-cobble-medallion"
  );
  ...
  for (const radius of [4.4, 6.9]) {
    const ring = mesh(
      owner,
      new THREE.TorusGeometry(radius, 0.08, 5, 40),
      mats.darkStone,
      "plaza-ring-course"
    );
  ...
  for (let i = 0; i < 16; i += 1) {
    ...
    const tile = mesh(
      owner,
      new THREE.ShapeGeometry(diamondShape(0.42)),
      i % 2 ? mats.darkStone : mats.plaster,
      "plaza-compass-inlay"
    );
    tile.rotation.x = -Math.PI / 2;
    tile.position.set(
      plaza.position.x + Math.cos(angle) * 5.55,
      0.08,
      plaza.position.z + Math.sin(angle) * 5.55
    );
...
function addTownRoutes(
  ...
  for (const route of data.routes) {
    ...
    const routeMesh = mesh(
      owner,
      strip,
      primary ? mats.route : mats.routeSecondary,
      `town-route:${route.id}`
    );
    ...
    for (let i = 1; i < points.length - 1; i += 1) {
      ...
      const marker = mesh(
        owner,
        new THREE.ShapeGeometry(diamondShape(primary ? 0.56 : 0.38)),
        mats.route,
        `route-mark:${route.id}:${i}`
      );
...
function addPerimeterFacades(
  ...
  for (let index = 0; index < EDGE_CANDIDATES.length; index += 1) {
    ...
    if (
      !isDecorativePocketClear(
        data,
        candidate.x,
        candidate.z,
        Math.max(dimensions.width, dimensions.depth) * 0.55 + 0.8
      )
    )
      continue;
...
function buildFacade(
  ...
  if (style === "gable") {
    const walls = mesh(
      owner,
      new THREE.BoxGeometry(width, height * 0.66, depth),
      mats.plaster,
      "timber-rowhouse-wall"
    );
    ...
    gable.lineTo(0, height * 0.37);
    gable.closePath();
    const peak = mesh(
      owner,
      new THREE.ExtrudeGeometry(gable, { depth, bevelEnabled: false }),
      mats.plaster,
      "gable-end"
    );
    ...
    const body = mesh(
      owner,
      new THREE.CylinderGeometry(width * 0.42, width * 0.47, height * 0.65, 12),
      mats.plaster,
      "curved-plaster-shop"
    );
    body.position.y = height * 0.325;
    body.scale.z = depth / width;
    group.add(body);
    const roof = mesh(
      owner,
      new THREE.ConeGeometry(width * 0.56, height * 0.39, 10),
      mats.roofBlue,
      "roundhouse-conical-roof"
    );
    roof.position.y = height * 0.83;
    roof.scale.z = depth / width;
    group.add(roof);
    addWarmWindows(group, width * 0.78, height, depth, mats, owner, 3);
    const belt = mesh(
      owner,
      new THREE.TorusGeometry(width * 0.42, 0.075, 5, 16),
      mats.timber,
      "roundhouse-timber-belt"
    );
    belt.position.y = height * 0.42;
    ...
    const counter = mesh(
      owner,
      new THREE.BoxGeometry(width * 0.8, height * 0.24, depth * 0.34),
      mats.timber,
      "market-counter"
    );
    counter.position.set(0, height * 0.12, depth * 0.36);
    group.add(counter);
    for (let i = 0; i < 5; i += 1) {
      const post = mesh(
        owner,
        new THREE.CylinderGeometry(0.07, 0.09, height * 0.78, 6),
        mats.timber,
        "awning-post"
      );
      post.position.set((i - 2) * width * 0.19, height * 0.39, depth * 0.36);
      group.add(post);
    }
    for (let i = 0; i < 6; i += 1) {
      const stripe = mesh(
        owner,
        new THREE.PlaneGeometry(width / 6 + 0.015, depth * 0.56),
        i % 2 ? mats.canvasGold : mats.canvasRed,
      ...
      const ware = mesh(
        owner,
        new THREE.SphereGeometry(0.18 + (i % 2) * 0.035, 6, 4),
        i % 3 ? mats.flower : mats.leafLight,
        "market-wares"
      );
      ware.position.set((i - 2.5) * width * 0.12, height * 0.3, depth * 0.54);
      group.add(ware);
    }
    const rear = mesh(
      owner,
      new THREE.BoxGeometry(width * 0.84, height * 0.32, 0.3),
      mats.plaster,
      "market-back-wall"
    );
    rear.position.set(0, height * 0.51, -depth * 0.25);
    group.add(rear);
  } else if (style === "tower") {
    const lower = mesh(
      owner,
      new THREE.CylinderGeometry(width * 0.38, width * 0.46, height * 0.7, 8),
      mats.darkStone,
      "watchtower-lower-storey"
    );
    lower.position.y = height * 0.35;
    lower.scale.z = depth / width;
    group.add(lower);
    const upper = mesh(
      owner,
      new THREE.CylinderGeometry(width * 0.31, width * 0.36, height * 0.42, 8),
      mats.plaster,
      "watchtower-upper-storey"
    );
    upper.position.y = height * 0.83;
    upper.scale.z = depth / width;
    group.add(upper);
    const cap = mesh(
      owner,
      new THREE.ConeGeometry(width * 0.48, height * 0.34, 8),
      mats.roof,
      "watchtower-cap"
    );
    cap.position.y = height * 1.17;
    ...
    body.scale.set(width * 0.48, height * 0.48, depth * 0.46);
    group.add(body);
    const roof = mesh(
      owner,
      new THREE.ConeGeometry(width * 0.52, height * 0.38, 7),
      mats.roofBlue,
      "root-cottage-roof"
    );
    roof.position.y = height * 0.88;
    roof.rotation.y = 0.2;
    group.add(roof);
    for (let i = 0; i < 7; i += 1) {
      ...
      const vine = mesh(
        ...
        new THREE.TubeGeometry(
          new THREE.CatmullRomCurve3([
            new THREE.Vector3(
              Math.cos(angle) * width * 0.17,
              height * 0.35,
              Math.sin(angle) * depth * 0.17
            ),
            new THREE.Vector3(
              Math.cos(angle + 0.16) * width * 0.34,
              height * 0.18,
              Math.sin(angle + 0.16) * depth * 0.34
            ),
            new THREE.Vector3(
              Math.cos(angle + 0.1) * width * 0.45,
              0.08,
              Math.sin(angle + 0.1) * depth * 0.45
            ),
          ]),
          8,
          0.06,
...
function addGabledRoof(
  ...
  for (const side of [-1, 1]) {
    const roof = mesh(
      owner,
      new THREE.BoxGeometry(width * 0.63, 0.18, depth * 1.14),
      roofColor,
      "slate-gable-roof-plane"
    );
    ...
    for (let row = 0; row < 3; row += 1) {
      const course = mesh(
        owner,
        new THREE.BoxGeometry(width * 0.66, 0.055, depth * 1.16),
        row % 2 ? mats.roofBlue : roofColor,
        "roof-shingle-course"
      );
      course.position.set(
        side * (width * 0.2 + row * width * 0.055),
        height * 0.91 - row * height * 0.035,
        0
      );
  ...
  const ridge = mesh(
    owner,
    new THREE.CylinderGeometry(0.12, 0.12, depth * 1.22, 6),
    mats.timber,
    "roof-ridge-beam"
  );
...
function addTimberGrid(
  ...
  for (const x of [-1, 1]) {
    const post = mesh(
      owner,
      new THREE.BoxGeometry(0.16, wallHeight, 0.18),
      mats.timber,
      "timber-facade-post"
    );
    post.position.set(x * width * 0.38, wallHeight / 2, depth / 2 + 0.06);
    group.add(post);
  }
  const cross = mesh(
    owner,
    new THREE.BoxGeometry(width * 0.92, 0.15, 0.19),
    mats.timber,
    "timber-facade-crossbeam"
  );
  cross.position.set(0, wallHeight * 0.45, depth / 2 + 0.06);
  group.add(cross);
  for (const side of [-1, 1]) {
    const brace = mesh(
      owner,
      new THREE.BoxGeometry(0.11, wallHeight * 0.42, 0.16),
      mats.timber,
      "timber-gable-brace"
    );
    brace.position.set(
...
function addWarmWindows(
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const frame = mesh(
      owner,
      new THREE.BoxGeometry(0.72, 0.95, 0.14),
      mats.timber,
      "deep-window-frame"
    );
    frame.position.set(x, y, depth * 0.47);
    group.add(frame);
    const pane = mesh(
      owner,
      new THREE.PlaneGeometry(0.49, 0.7),
      mats.windowGlow,
      "warm-window-pane"
    );
    pane.position.set(x, y, depth * 0.55);
    group.add(pane);
    const mullion = mesh(
      owner,
      new THREE.BoxGeometry(0.07, 0.72, 0.08),
      mats.darkStone,
      "window-mullion"
    );
    mullion.position.set(x, y, depth * 0.57);
    group.add(mullion);
    const sill = mesh(
      owner,
      new THREE.BoxGeometry(0.88, 0.11, 0.24),
      mats.stone,
      "stone-window-sill"
    );
    sill.position.set(x, y - 0.53, depth * 0.52);
    group.add(sill);
  }
}
...
function addDoorway(
  ...
  const door = mesh(
    owner,
    new THREE.BoxGeometry(width * 0.2, height * 0.34, 0.16),
    mats.timber,
    "deep-set-shop-door"
  );
  door.position.set(0, height * 0.17, depth / 2 + 0.09);
  group.add(door);
  const arch = mesh(
    owner,
    new THREE.TorusGeometry(width * 0.12, 0.07, 5, 12, Math.PI),
    mats.stone,
    "door-stone-arch"
  );
  arch.position.set(0, height * 0.34, depth / 2 + 0.1);
  group.add(arch);
  const lamp = mesh(
    owner,
  ...
  lamp.position.set(width * 0.17, height * 0.42, depth / 2 + 0.14);
  group.add(lamp);
}

function addStreetGreenery(
  group: THREE.Group,
  data: Floor7Town,
  mats: TownMaterials,
  owner: ReturnType<typeof ownResources>
  ...
  for (const [x, z] of candidates) {
    ...
    const planter = mesh(
      owner,
      new THREE.CylinderGeometry(0.85, 0.72, 0.42, 9),
      index % 2 ? mats.stone : mats.darkStone,
      "low-stone-planter"
    );
    ...
    for (let leafIndex = 0; leafIndex < 7; leafIndex += 1) {
      ...
      const shrub = mesh(
        owner,
        new THREE.IcosahedronGeometry(0.52 + (leafIndex % 3) * 0.08, 0),
        leafIndex % 3 ? mats.leaf : mats.leafLight,
        "planter-foliage-clump"
      );
      shrub.position.set(
        x + Math.cos(angle) * 0.45,
        0.64 + (leafIndex % 2) * 0.12,
...
function addStreetFixtures(
  ...
  for (const [x, z] of candidates) {
    ...
    const pole = mesh(
      owner,
      new THREE.CylinderGeometry(0.09, 0.15, 3.5, 7),
      mats.timber,
      "street-lamp-post"
    );
    ...
    const arm = mesh(
      owner,
      new THREE.CylinderGeometry(0.045, 0.055, 0.72, 6),
      mats.timber,
      "curved-lantern-arm"
    );
    arm.position.set(x + 0.26, 3.42, z);
    arm.rotation.z = Math.PI / 2;
    group.add(arm);
    const shade = mesh(
      owner,
      new THREE.CylinderGeometry(0.22, 0.32, 0.36, 6),
      mats.darkStone,
      "lantern-cap"
    );
    shade.position.set(x + 0.55, 3.2, z);
    group.add(shade);
    const light = mesh(
      owner,
    ...
    light.position.set(x + 0.55, 2.98, z);
    group.add(light);
    index += 1;
  }
}

function isDecorativePocketClear(
  data: Floor7Town,
  x: number,
  z: number,
  radius: number
  ...
  if (
    Math.abs(x - data.bounds.x) + radius > halfWidth - 0.4 ||
    Math.abs(z - data.bounds.z) + radius > halfDepth - 0.4
  )
    return false;
  for (const route of data.routes) {
    const points = [route.from, ...route.waypoints, route.to];
    for (let i = 0; i < points.length - 1; i += 1) {
      if (
        distanceToSegment({ x, z }, points[i]!, points[i + 1]!) <
        route.width / 2 + radius + 0.8
      )
        return false;
    }
  }
  ...
  for (const encounter of data.encounters) {
    const dx = Math.max(
      Math.abs(x - encounter.collision.x) - encounter.collision.width / 2,
      0
    );
    const dz = Math.max(
      Math.abs(z - encounter.collision.z) - encounter.collision.depth / 2,
      0
    );
  ...
  for (const landmark of data.landmarks) {
    if (
      Math.hypot(x - landmark.position.x, z - landmark.position.z) <
      radius + 1.8
    )
      return false;
  }
...
function townRouteGeometry(
  points: readonly LateFloorPoint[],
  width: number
