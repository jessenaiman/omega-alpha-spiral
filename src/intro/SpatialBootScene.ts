...
import {
  loadIntroBlenderExtras,
  type IntroBlenderExtras,
} from "./IntroBlenderExtras";
import {
  INTRO_JOURNEY_SENSOR_INDEX,
  IntroPhysics,
  type IntroPhysicsDiagnostics,
...
function wrapAtWords(text: string, columns: number): string {
  return text
    .split("\n")
    .map((sourceLine: string): string => {
      let line = sourceLine;
...
function createDreamweaverMark(owner: number): DreamweaverMark {
  ...
  if (owner === 0) {
    const segments: number[][] = [
      [-0.32, 0, 0, 0.32, 0, 0],
      [0, -0.32, 0, 0, 0.32, 0],
      [-0.23, -0.23, 0, 0.23, 0.23, 0],
      [-0.23, 0.23, 0, 0.23, -0.23, 0],
    ];
    geometry.setAttribute(
      "position",
      new BufferAttribute(new Float32Array(segments.flat()), 3)
    );
  } else if (owner === 1) {
    points.push(
      new Vector3(-0.34, 0.28, 0),
      new Vector3(-0.04, 0.12, 0),
      new Vector3(-0.27, -0.03, 0),
      new Vector3(0.08, -0.13, 0),
      new Vector3(-0.03, -0.34, 0),
      new Vector3(0.34, -0.19, 0)
    );
  ...
  const main: LineBasicMaterial = new LineBasicMaterial({
    color: INK[owner],
    transparent: true,
    opacity: 0.96,
    toneMapped: false,
  });
  const halo: LineBasicMaterial = new LineBasicMaterial({
    color: INK[owner],
    transparent: true,
    opacity: 0.2,
  ...
  const line: Line | LineSegments =
    owner === 0 ? new LineSegments(geometry, main) : new Line(geometry, main);
  const glow: Line | LineSegments =
    owner === 0 ? new LineSegments(geometry, halo) : new Line(geometry, halo);
  ...
  const core: Mesh<BoxGeometry, MeshStandardMaterial> = new Mesh(
    new BoxGeometry(0.085, 0.085, 0.11),
    coreMaterial
  );
  const depthColor: number[] = [0x5b8ca8, 0x050103, 0x3f2a78];
  const depthMaterial: LineBasicMaterial = new LineBasicMaterial({
    color: depthColor[owner],
    transparent: true,
    opacity: 0.38,
    depthWrite: false,
    toneMapped: false,
  });
  const depthLine: Line | LineSegments =
    owner === 0
      ? new LineSegments(geometry, depthMaterial)
      : new Line(geometry, depthMaterial);
  depthLine.position.set(
    owner === 1 ? -0.08 : 0,
    owner === 2 ? 0.04 : 0,
    -0.18
  );
  depthLine.scale.setScalar(owner === 0 ? 0.72 : owner === 1 ? 1.12 : 0.78);
  const practical: PointLight = new PointLight(
    INK[owner],
    owner === 1 ? 2.6 : 3.4,
    2.8,
    2
  ...
  if (owner === 1) {
    ...
    for (let index: number = 1; index < points.length; index += 1) {
      ...
      const segment: Mesh<BoxGeometry, MeshStandardMaterial> = new Mesh(
        new BoxGeometry(Math.hypot(dx, dy), 0.048, 0.13),
        shadowSurface
      );
...
export function createAtlas(format: number): CanvasTexture {
  ...
  const cell: number = Math.max(
    10,
    Math.round(era.cell * (format === 0 ? BOOT_EFFECTS.pixelStep : 1))
  );
...
export class GlyphRibbon {
  ...
  constructor(atlases: CanvasTexture[]) {
    ...
    const uniforms = {
      ...
      uEra: { value: 0 },
      uTime: { value: 0 },
      uScanline: { value: era.scanline },
      uDamage: { value: era.damage },
      uBitDepth: { value: era.bitDepth },
    };
  ...
  public setText(
    ...
    this._front.uniforms.uUseOverride.value =
      color !== undefined || ownerStyle >= 0 ? 1 : 0;
    this._back.uniforms.uColor.value.setHex(
      ownerStyle >= 0 ? BACK_INK[ownerStyle] : era.echo
    );
    if (
      this._text === text &&
      this._format === format &&
  ...
  public update(seconds: number, disorder: number): void {
    ...
    for (const letter of this._text) {
      ...
      const drift: number =
        ownerStyle === 0
          ? 0
          : Math.sin(seconds * 0.32 + glyph * 0.21) * authoredDisorder;
      let z: number =
        ownerStyle === 0
          ? 0
          : Math.sin(glyph * 0.71 + seconds * 0.2) *
            (ownerStyle >= 0 ? authoredDisorder * 0.6 : era.depth);
      let skew: number =
        ownerStyle === 0 ? 0 : Math.cos(glyph * 0.37) * authoredDisorder * 0.18;
      if (ownerStyle === 1) {
      ...
      if (ownerStyle === 2) {
        const arc: number = Math.sin(
          Math.min(1, column / Math.max(1, this._columns - 1)) * Math.PI
        );
        x += arc * 0.2;
...
export class SpatialBootScene {
  ...
  private _walkPlane: Group = new Group();
  ...
  private _guidedChoice: number = -1;
  private _doorGuided: boolean = false;
  private _doorCrossed: boolean = false;
  ...
  private _journeyStart: Vector3 = new Vector3();
  private _journeyStartY: number = 0;
  private _stationDepth: number = 0;
  ...
  private _fossils: Array<{
    text: string;
    era: number;
    color: number;
    owner: number;
  }> = [];
  private _voiceStarts: Vector3[] = [
    new Vector3(),
    new Vector3(),
    new Vector3(),
  ];
  private _voiceResting: Vector3[] = [
    new Vector3(),
    new Vector3(),
    new Vector3(),
  ];
  ...
  public init(scene: Scene): Promise<void> {
    ...
    for (let index: number = 0; index < 3; index += 1) {
      ...
      this._ribbons[14 + index].setText(
        `[ ${VOICE_NAMES[index]} ]`,
        index + 1,
        24,
        INK[index],
    ...
    const layersReady = loadBlenderIntroLayers(this._root, "game")
      .then((layers) => {
        if (this._destroyed) return;
        this._blenderLayers = layers;
        this._blenderStars =
          layers.background.getObjectByName("VoidStars_MESH") ?? null;
      })
      .catch((error: unknown) => {
    ...
    const extrasReady = loadIntroBlenderExtras(this._root)
      ...
      .catch((error: unknown): void => {
        console.warn("Unable to load Blender threshold extras", error);
      });
    const doorReady = new GLTFLoader()
      .loadAsync("/assets/intro/intro-door-assemble.glb")
      .then(({ scene: door }: { scene: Group }): void => {
        if (this._destroyed) return;
        door.visible = false;
        door.position.set(0, -1.82, 0);
        door.scale.setScalar(0.88);
        door.rotation.y = -0.08;
        const doorColors: Record<string, number> = {
          Intro_Copper: 0x9f704b,
          Intro_Cyan: 0x8ce9ff,
          Intro_Basalt: 0x344b58,
          Intro_Amber: 0xf1ac62,
          Intro_Iron: 0x829ca8,
          Intro_Obsidian: 0x1c313e,
          Intro_Void: 0x02060c,
        };
        const doorMaterials = new Map<string, MeshBasicMaterial>();
        let fragmentIndex = 0;
        door.traverse((node: Object3D): void => {
          if (node instanceof Mesh) {
            ...
            const replacements = originals.map(
              (material): MeshBasicMaterial => {
                ...
                let replacement = doorMaterials.get(name);
                if (!replacement) {
                  replacement = new MeshBasicMaterial({
                    color: doorColors[name] ?? 0x526776,
                    toneMapped: false,
                  });
                  doorMaterials.set(name, replacement);
                }
                return replacement;
              }
            );
          ...
          if (node.name === "Art_VoidBacking") node.visible = false;
          if (!node.name.startsWith("DoorFragment_")) return;
          node.position.x += node.name.includes("_L_") ? -0.55 : 0.55;
          const angle = fragmentIndex * 2.39996;
          this._doorFragments.push({
            node,
            home: node.position.clone(),
            offset: new Vector3(
              Math.cos(angle) * 1.7,
              Math.sin(angle) * 1.25,
              -0.8 - (fragmentIndex % 5) * 0.17
            ),
          });
          fragmentIndex += 1;
        });
        const addRail = (
          ...
          const rail = new Mesh(
            new BoxGeometry(width, height, 0.035),
            material
          );
          rail.position.set(x, y, 0.9);
          rail.renderOrder = 8;
          this._doorRailMaterials.push(material);
          door.add(rail);
        };
        addRail(0.024, 3.18, -1.16, 1.77, 0x83dbea);
        addRail(0.024, 3.18, 1.16, 1.77, 0x83dbea);
        addRail(2.34, 0.024, 0, 3.35, 0x83dbea);
        addRail(0.018, 3.34, -1.58, 1.78, 0xc08351);
        addRail(0.018, 3.34, 1.58, 1.78, 0xc08351);
        this._door = door;
        this._root.add(door);
      })
  ...
  private _createPlayer(): void {
    ...
    this._playerCore = new Mesh(
      new BoxGeometry(0.18, 0.18, 0.18),
      coreMaterial
    );
    this._playerCore.position.y = 0.35;
    const layerSegments: number[] = [
      // Stage 1: cursor rails.
      -0.13, -0.31, 0, -0.13, 0.31, 0, 0.13, -0.31, 0, 0.13, 0.31, 0, -0.13,
      0.31, 0, 0.13, 0.31, 0, -0.13, -0.31, 0, 0.13, -0.31, 0,
      // Stage 2: a decision diamond.
      0, 0.43, 0, 0.3, 0, 0, 0.3, 0, 0, 0, -0.43, 0, 0, -0.43, 0, -0.3, 0, 0,
      -0.3, 0, 0, 0, 0.43, 0,
      // Stage 3: depth rails.
      -0.3, 0, 0, -0.2, 0, 0.24, 0.3, 0, 0, 0.2, 0, 0.24, 0, 0.43, 0, 0, 0.28,
      0.24, 0, -0.43, 0, 0, -0.28, 0.24,
      // Stage 4: an open threshold, deliberately missing its lower edge.
      -0.34, -0.34, 0.04, -0.34, 0.48, 0.04, -0.34, 0.48, 0.04, 0.34, 0.48,
      0.04, 0.34, 0.48, 0.04, 0.34, -0.34, 0.04, -0.2, 0.32, 0.04, 0.2, 0.32,
      0.04, -0.2, 0.12, 0.04, 0.2, 0.12, 0.04, -0.2, -0.08, 0.04, 0.2, -0.08,
      0.04,
    ];
    const outlineGeometry: BufferGeometry = new BufferGeometry();
    outlineGeometry.setAttribute(
      "position",
      new BufferAttribute(new Float32Array(layerSegments), 3)
    );
    outlineGeometry.setDrawRange(0, 0);
    const outlineMaterial: LineBasicMaterial = new LineBasicMaterial({
      color: 0xc9edff,
      transparent: true,
      opacity: 0.78,
      blending: AdditiveBlending,
  ...
  private _createPath(): void {
    ...
    for (let owner: number = 0; owner < 3; owner += 1) {
      ...
      const echo: LineSegments<BufferGeometry, LineBasicMaterial> =
        new LineSegments(geometry, echoMaterial);
      echo.frustumCulled = false;
      echo.position.set(
        owner === 2 ? 0.11 : owner === 1 ? -0.055 : 0,
        0,
        owner === 2 ? -0.1 : -0.055
      );
  ...
  public beginJourney(): void {
    ...
    // Continue from the Dreamweaver the player actually reached. The physics
    // body measures progress; the visible avatar follows that strand in depth.
    this._journeyStart.copy(this._player.position);
    this._journeyStartY = this._player.position.y;
    this._physics.activateJourney(
      ...
      { x: this._player.position.x, y: 4, z: this._player.position.z }
    );
  }
  ...
  public getWorldDepth(): number {
    return this._stationDepth;
  }

  public beginDoorway(_name: string): void {
    this._doorGuided = false;
  ...
  public guideThroughDoor(): void {
    this._doorGuided = true;
  }

  public setPlayerStage(stage: number): void {
    this._playerStage = Math.max(0, Math.min(4, Math.floor(stage)));
    this._avatar.setStage(this._playerStage);
    const drawCount: number[] = [0, 8, 16, 24, 36];
    this._playerOutline?.geometry.setDrawRange(0, drawCount[this._playerStage]);
    if (this._playerCore)
      this._playerCore.scale.setScalar(0.72 + this._playerStage * 0.1);
    this._playerMemories.forEach(
      (
        memory: Mesh<BoxGeometry, MeshStandardMaterial>,
        index: number
      ): void => {
  ...
  public setFrame(frame: BootFrame): void {
    ...
    this._ribbons[0].setText(
      frame.phase === "command" || frame.phase === "cursor"
        ? frame.transcript
        : transcriptLines
            .filter((line: string): boolean => !line.startsWith("dreamweaver["))
            .join("\n"),
      frame.format,
      columns
    );
    for (let index: number = 0; index < 3; index += 1) {
      this._ribbons[index + 1].setText(
        transcriptLines.find((line: string): boolean =>
          line.startsWith(`dreamweaver[0${index + 1}]`)
        ) ?? "",
        index,
        columns,
        INK[index],
    ...
    const speakingOwner: number =
      frame.phase === "response"
        ? Math.max(0, frame.speaker ?? this._selected)
        : -1;
    const responseColor: number =
      speakingOwner >= 0 ? INK[speakingOwner] : INK[0];
    ...
    this._ribbons[5].setText(
      wrapAtWords(frame.question, questionColumns),
      responseOwner >= 0 ? responseOwner + 1 : frame.format,
      questionColumns,
      responseColor,
      responseOwner
    );
    ...
    const asideVoice: number = frame.hint
      ? Math.max(0, Math.min(5, frame.format))
  ...
  public getChoiceTargets(): Array<{ x: number; y: number; z: number }> {
    return this._targets.map(
      (
        target: Mesh<BoxGeometry, MeshBasicMaterial>
  ...
  public guideToChoice(index: number): void {
    this._guidedChoice = index >= 0 && index < 3 ? index : -1;
    if (this._guidedChoice >= 0) {
      this._selected = this._guidedChoice;
      this._hovered = this._guidedChoice;
      this._particles.pulse(this._guidedChoice);
    }
  }
  ...
  public getPhysicsDiagnostics(): IntroPhysicsDiagnostics {
    return this._physics.getDiagnostics();
  }

  public settleForTestState(elapsedMs: number): void {
    this._avatar.settle();
    this._voiceAppearedAt = [
      elapsedMs - 3200,
      elapsedMs - 3200,
      elapsedMs - 3200,
    ];
  ...
  public setStoryProgressForTest(choicesMade: number): void {
    this._choiceHistory = [0, 1, 2, 0].slice(
      0,
    ...
    this._retunePlayer();
    this._avatar.settle();
    this._worldEvolution.settle(this._choiceHistory.length);
  }
  ...
  public reset(): void {
    ...
    this._journeyActive = false;
    this._journeyComplete = false;
    this._stationDepth = 0;
    this._root.position.z = 0;
    this._choiceArmed = false;
    this._guidedChoice = -1;
    this._doorGuided = false;
    this._doorCrossed = false;
    this._physicsX = 0;
    this._player.position.set(0, -2.55, 0.52);
    ...
    this._worldEvolution.reset();
    this._physics.reset();
  }

  public commitChoice(index: number): void {
    const target: Mesh<BoxGeometry, MeshBasicMaterial> | undefined =
      this._targets[index];
    if (target)
      this._player.position.set(
        target.position.x,
        target.position.y - 0.46,
        0.56
      );
    this._choiceHistory.push(Math.max(0, Math.min(2, index)));
    this._awaitingAction = -1;
    const memory: Mesh<BoxGeometry, MeshStandardMaterial> | undefined =
      this._playerMemories[
        Math.min(this._playerStage, this._playerMemories.length - 1)
      ];
    if (memory) {
      ...
      memory.rotation.z =
        index === 0
          ? 0
          : index === 1
            ? this._playerStage % 2 === 0
              ? 0.62
              : -0.82
            : 0.34 + this._playerStage * 0.38;
      memory.visible = true;
    }
    this._retunePlayer();
    this._choiceArmed = false;
  ...
  private _retunePlayer(): void {
    ...
    this._choiceHistory.forEach((owner: number): void => {
      imprint.add(new Color(INK[owner]));
    });
  ...
  private _setVoiceResting(
    ...
    const breath: number = still
      ? 0
      : Math.sin(seconds * (0.24 + index * 0.025) + index) * 0.22;
    if (era <= 1) {
    ...
    target.set(
      order * lane * 0.82,
      index === 2 ? 2.55 : 1.88 + breath,
      0.12 + (index === 2 ? 0 : 0.2)
    );
  }

  private _writePathSegment(
    owner: number,
    vertex: number,
    ax: number,
    ay: number,
  ...
  private _samplePath(
      ...
      const x: number =
        segment === 0
          ...
          : segment === 1
            ? x1 + (x2 - x1) * local
            : x2 + (goal.x - x2) * local;
      const steppedZ: number =
        from.z + (goal.z - from.z) * t - Math.floor(t * 4) * 0.035;
      return out.set(x, y, steppedZ);
    }
    const inverse: number = 1 - t;
    const direction: number =
      goal.x === from.x ? 1 : Math.sign(goal.x - from.x);
    ...
    const x: number =
      ...
      t * t * t * goal.x;
    const desire: number =
      Math.sin(t * Math.PI) * (0.3 + Math.sin(seconds * 0.25) * 0.035);
    return out.set(
      x,
      y + desire * 0.18,
      from.z + (goal.z - from.z) * t + desire
    );
  }

  private _createWalkPlane(): void {
    // The transparent substrate recedes in real depth under the traversed strands.
    const nearY = -2.72;
    const farY = 2.72;
    const depth = (y: number): number => 0.03 - (y - nearY) * 0.62;
    ...
    for (let index = -6; index <= 6; index += 1) {
      const x = index * 0.56;
      lines.push(x, nearY, depth(nearY) + 0.015, x, farY, depth(farY) + 0.015);
    }
    for (let index = 0; index <= 11; index += 1) {
      const y = nearY + ((farY - nearY) * index) / 11;
      lines.push(-3.65, y, depth(y) + 0.015, 3.65, y, depth(y) + 0.015);
    }
    ...
    this._root.add(this._walkPlane);
  }
  ...
  private _updatePath(
    ...
    for (let owner: number = 0; owner < 3; owner += 1) {
      const path: LineSegments<BufferGeometry, LineBasicMaterial> =
        this._paths[owner];
      const echo: LineSegments<BufferGeometry, LineBasicMaterial> =
        this._pathEchoes[owner];
      ...
      if (isThreshold) {
        ...
        this._pathGoal.set(
          0,
          -2.45 + crossingProgress * 2.58,
          0.34 - crossingProgress * 3.28
        );
      } else {
        if (activeTravel) this._pathFrom.copy(this._journeyStart);
        else this._pathFrom.set(0, -2.62, 0.42);
      }
      if (activeTravel) this._pathGoal.set(0, -2.55, -6);
      else {
        if (!isThreshold) {
      ...
      for (let sample: number = 1; sample <= samples; sample += 1) {
        ...
        vertex = this._writePathSegment(
          owner,
          vertex,
          this._pathPrevious.x,
          this._pathPrevious.y,
          this._pathPrevious.z,
          this._pathCurrent.x,
          this._pathCurrent.y,
          this._pathCurrent.z
        );
        if (
          sample % (owner === 1 ? 3 : 4) === 0 &&
          vertex + 2 <= PATH_VERTEX_CAPACITY
        ) {
          ...
          const width: number =
            owner === 0
              ...
              : owner === 1
                ? 0.075
                : 0.095 + Math.sin(t * Math.PI) * 0.045;
          const px: number = (-dy / length) * width;
          const py: number = (dx / length) * width;
          vertex = this._writePathSegment(
            owner,
            vertex,
            this._pathCurrent.x - px,
            this._pathCurrent.y - py,
            this._pathCurrent.z,
            this._pathCurrent.x + px,
            this._pathCurrent.y + py,
            this._pathCurrent.z
      ...
      const callProgress: number =
        !activeCall || appearedAt < 0
          ? 1
          : Math.min(1, Math.max(0, (elapsedMs - appearedAt) / 2500));
      const revealedVertices: number = Math.max(
        0,
        Math.floor((vertex * callProgress) / 2) * 2
      );
      const attribute: BufferAttribute = path.geometry.getAttribute(
        "position"
      ) as BufferAttribute;
      ...
      const resonance: number =
        this._choiceHistory.length < 2
          ? 0
          : this._choiceHistory.filter(
              (choice: number): boolean => choice === owner
            ).length / this._choiceHistory.length;
      const emphasis: number =
        activeCall || activeTravel || isThreshold ? 1 : hovered ? 0.78 : 0.34;
      path.material.opacity =
        (0.16 + emphasis * 0.42 + resonance * 0.1) * (owner === 1 ? 0.82 : 1);
      echo.material.opacity =
        emphasis * (owner === 2 ? 0.34 : owner === 1 ? 0.22 : 0.12) +
        resonance * 0.16;
      const lightProgress: number = isThreshold
        ? Math.min(1, 0.22 + crossingProgress * 0.78)
        : activeCall
          ? callProgress
          : hovered || activeTravel
            ? Math.sin(seconds * 0.72) * 0.5 + 0.5
            : 0;
      this._samplePath(
        owner,
        lightProgress,
      ...
      this._pathLights[owner].intensity =
        emphasis *
        (isThreshold
          ? 6.4
          : activeCall
  ...
  public update(elapsedMs: number, isReduced: boolean): number {
    ...
    if (this._blenderLayers) {
      ...
      this._blenderLayers.background.visible =
        showBackground && this._choiceHistory.length > 0;
      this._blenderLayers.strands.visible =
        frame.phase === "travel" ||
        (frame.phase === "waiting" && this._choiceHistory.length > 0);
    ...
    const delta: number =
      this._lastUpdateMs === 0
        ? 0
        : Math.min(0.1, Math.max(0, (elapsedMs - this._lastUpdateMs) / 1000));
    this._lastUpdateMs = elapsedMs;
    this._avatar.update(
      delta,
      seconds,
      frame.phase === "travel" || frame.phase === "complete"
    );
    ...
    const isStory: boolean =
      ...
      frame.phase !== "loading" &&
      frame.phase !== "writing";
    const isName: boolean = frame.phase === "name";
    ...
    if (this._blenderExtras) {
      this._blenderExtras.portal.visible =
    ...
    if (phaseChanged) {
      ...
      if (isName) this._player.position.set(0, -2.45, 0.52);
    ...
    const disorder: number =
      isReduced || isSettled ? 0 : frame.isCorrupt ? 1.2 : 0.18;
    ...
    const formation: number = isName ? 0.65 : isThreshold ? 1 : 0;
    ...
    if (!isReduced && !isWaiting) {
      ...
      command.rotation.y =
        Math.sin(seconds * 0.23) *
        BOOT_EFFECTS.planeSkew *
        (isBoot ? 0.48 : 0.18);
    ...
    const loadedVoices: boolean[] = [0, 1, 2].map((index: number): boolean =>
      frame.transcript.includes(`dreamweaver[0${index + 1}]`)
    );
    ...
    for (let index: number = 0; index < 3; index += 1) {
      ...
      const isSpeaking: boolean =
        (frame.phase === "response" &&
          (frame.speaker ?? this._selected) === index) ||
        (this._voice === index && elapsedMs - this._asideAt < ASIDE_HOLD_MS);
      const isNewestArrival: boolean =
        frame.phase === "loading" && newestVoice === index;
      this._setVoiceResting(
        resting,
        index,
        frame.format,
        seconds,
      ...
      else if (smoothEntrance < 0.999)
        voice.position.copy(start).lerp(resting, smoothEntrance);
      else
        voice.position.lerp(
          resting,
          Math.min(1, delta * (isTravel ? 0.78 : 1.45))
        );
      if (index === 0)
        voice.rotation.set(
          0,
          0,
          isReduced || isWaiting || isSpeaking
            ? 0
            : Math.sin(seconds * 0.2) * 0.025
        );
      if (index === 1)
        voice.rotation.set(
          0,
          ...
          isReduced || isWaiting
            ? -0.12
            : ((Math.floor(seconds * 0.6) % 5) - 2) * 0.08
        );
      if (index === 2)
        voice.rotation.set(
          0,
          0,
          isReduced || isWaiting ? 0.08 : seconds * 0.16
        );
      const speakingPulse: number = isReduced
        ? 1
        : 1 + Math.sin(seconds * 5.4) * 0.1;
      ...
      this._voiceMaterials[index][0].opacity =
        (isSpeaking ? 1 : 0.88) * observerFade;
      this._voiceMaterials[index][1].opacity =
        (isSpeaking ? 0.48 : 0.16) * observerFade;
      this._voiceMaterials[index][2].opacity =
        (isSpeaking ? 1 : 0.72) * observerFade;
      this._voiceMaterials[index][3].opacity =
        (isSpeaking ? 0.58 : index === 1 ? 0.08 : 0.28) * observerFade;
      if (this._voiceMaterials[index][4])
        this._voiceMaterials[index][4].opacity = isSpeaking ? 0.82 : 0.18;
      ...
      for (let pointIndex: number = 0; pointIndex < 72; pointIndex += 1) {
        ...
        const point: Vector3 = this._trailPoint
          .copy(start)
          .lerp(voice.position, t);
        if (index === 0) {
          point.z -= (1 - t) * 0.35;
        } else {
      ...
      trail.material.opacity =
        (0.12 + smoothEntrance * (isNewestArrival ? 0.48 : 0.24)) *
        observerFade *
    ...
    const responseOwner: number =
      frame.phase === "response"
        ? Math.max(0, frame.speaker ?? this._selected)
        : -1;
    const responseAnchor: Vector3 | null =
      responseOwner >= 0 ? this._voices[responseOwner].position : null;
    ...
    question.scale.setScalar(
      frame.phase === "doorway" || frame.phase === "complete"
        ? scale * (this._isNarrow ? 0.68 : 0.77)
        : isName
          ? scale * (this._isNarrow ? 0.92 : 1.16)
          : isThreshold
    ...
    for (let index: number = 0; index < 3; index += 1) {
      ...
      const focused =
        this._awaitingAction >= 0
          ? this._awaitingAction
          : this._hovered >= 0
            ? this._hovered
            : 1;
      ...
      this._pathFrom.set(0, -2.62, 0.42);
      this._pathGoal.set(target.position.x, target.position.y - 0.48, -2.1);
      this._samplePath(
        index,
        0.64,
        this._pathFrom,
      ...
      choice.position.set(left + this._width * 0.14, -1.12, 0.9);
      ...
      speaker.position.set(
        voicePosition.x - 0.48,
        voicePosition.y - 0.46,
        voicePosition.z + 0.04
      );
    ...
    if (isWaiting) {
      if (phaseChanged)
        this._physics.activateAnswers(this._physicsTargets, {
          x: 0,
          y: -2.55,
          z: 0.52,
        });
    ...
    for (let index: number = 0; index < 3; index += 1) {
      ...
      if (isTravel) {
        fossil.position.set(
          -this._width * 0.18 + index * this._width * 0.12,
          -1.48 + index * 0.72,
          -0.08 - index * 0.16
        );
    ...
    if (this._player.visible) {
      if (!isReduced) {
        this._player.rotation.y =
          Math.sin(seconds * 0.6) * (0.12 + this._playerStage * 0.025);
      ...
      if (isWaiting) {
        ...
        const guideX: number =
          this._guidedChoice >= 0
            ? this._targets[this._guidedChoice].position.x - this._physicsX
            : 0;
        const physicsStep: IntroPhysicsStep = this._physics.step(
          delta,
          this._guidedChoice >= 0
            ? Math.abs(guideX) < 0.07
              ? 0
              : Math.sign(guideX)
            : this._movement.x,
          this._guidedChoice >= 0 ? 1 : this._movement.y,
          speed,
          {
            minX: -this._width * 0.43,
            maxX: this._width * 0.43,
            minY: -2.65,
            maxY: 1.62,
          }
        );
        this._physicsX = physicsStep.position.x;
        let nearest: number = 0;
        let nearestDistance: number = Number.POSITIVE_INFINITY;
        for (let index: number = 0; index < 3; index += 1) {
          const dx: number = Math.abs(
            physicsStep.position.x - this._targets[index].position.x
          );
        ...
        if (this._guidedChoice >= 0) nearest = this._guidedChoice;
        this._hovered = nearest;
        const target: Vector3 = this._targets[nearest].position;
        const progress: number = Math.min(
          1,
          Math.max(0, (physicsStep.position.y + 2.65) / 4.27)
        );
        const lockRaw: number = Math.min(
          1,
          Math.max(0, (progress - 0.08) / 0.42)
        );
        const laneLock: number = lockRaw * lockRaw * (3 - 2 * lockRaw);
        this._pathFrom.set(0, -2.65, 0.42);
        this._pathGoal.set(target.x, target.y - 0.48, -2.1);
        this._samplePath(
          nearest,
          progress,
        ...
        this._player.position.set(
          physicsStep.position.x * (1 - laneLock) +
            this._pathCurrent.x * laneLock,
          physicsStep.position.y,
          physicsStep.position.z * (1 - laneLock) +
            (this._pathCurrent.z + 0.18) * laneLock
        );
        if (
          this._choiceArmed &&
          physicsStep.sensor >= 0 &&
          physicsStep.sensor < 3
        ) {
          this._choiceArmed = false;
        ...
        const progress: number = Math.min(
          ...
          Math.max(
            0,
            (physicsStep.position.y - this._journeyStartY) /
              (4 - this._journeyStartY)
          )
        );
        this._pathFrom.copy(this._journeyStart);
        this._pathGoal.set(0, -2.55, -6);
        this._samplePath(
          Math.max(0, this._selected),
          progress,
          this._pathFrom,
          this._pathGoal,
          seconds,
        ...
        const doorStep: IntroPhysicsStep = this._physics.step(
          delta,
          this._doorGuided ? 0 : this._movement.x * 0.36,
          this._doorGuided ? 1 : this._movement.y,
          1.66,
          { minX: -0.7, maxX: 0.7, minY: -2.45, maxY: 0.9 }
        );
        const approach: number = Math.min(
          1,
          Math.max(0, (doorStep.position.y + 2.45) / 3.27)
        );
        this._player.position.set(
          doorStep.position.x,
          doorStep.position.y,
        ...
        if (
          !this._doorCrossed &&
          doorStep.sensor === INTRO_JOURNEY_SENSOR_INDEX
        ) {
          this._doorCrossed = true;
          spatialEvent = -3;
        }
      } else if (isCrossing) {
        const eased: number =
          crossingProgress * crossingProgress * (3 - 2 * crossingProgress);
        this._player.position.set(0, -2.45 + eased * 2.58, 0.52 - eased * 3.46);
      }
      const waitingPulse: number =
        this._awaitingAction >= 0 && !isReduced
          ? 1 + Math.sin(seconds * 6.4) * 0.09
          : 1;
      const pulse: number =
    ...
    const stagedChoice: Group | null =
      isWaiting && this._awaitingAction >= 0
        ? this._ribbons[this._awaitingAction + 8].root
        : null;
    this._cursor.visible =
      (!isWaiting || Boolean(stagedChoice)) &&
      frame.phase !== "travel" &&
      frame.phase !== "name" &&
      frame.phase !== "doorway" &&
      frame.phase !== "complete" &&
    ...
    const cursorSource: string = stagedChoice
      ? (frame.choices[this._awaitingAction] ?? "")
      : isBoot
        ? frame.transcript
        : frame.question;
    const cursorLines: string[] = cursorSource.split("\n");
    const lastLine: string = cursorLines.at(-1) ?? "";
    const cursorTextScale: number = stagedChoice
      ? 0.39
      : isBoot
        ? 0.48
        : responseAnchor
    ...
    this._cursor.position.y -=
      Math.max(0, cursorLines.length - 1) * scale * 1.35 * cursorTextScale;
    ...
    const cursorScale: number = stagedChoice
      ? 0.44
      : isBoot
        ? 0.48
        : responseAnchor
    ...
    this._cursor.scale.set(
      (frame.format === 1 ? 0.14 : 1) * cursorScale,
      (frame.format === 2 ? 0.14 : 1) * cursorScale,
      cursorScale
    );
    this._cursor.material.color.setHex(
      this._awaitingAction >= 0
        ? INK[this._awaitingAction]
        : responseOwner >= 0
          ? INK[responseOwner]
          : INK[0]
    );
  ...
  public destroy(): void {
    ...
    this._door?.traverse((child): void => {
      if (!(child instanceof Mesh)) return;
      child.geometry.dispose();
      if (Array.isArray(child.material))
        child.material.forEach((material): void => material.dispose());
      else child.material.dispose();
    });
    this._walkPlane.traverse((child): void => {
    ...
    this._voices.forEach((voice: Group): void => {
      voice.traverse((child): void => {
        ...
        if (Array.isArray(material))
          material.forEach((entry): void => entry.dispose());
    ...
    this._playerMemories.forEach(
      (memory: Mesh<BoxGeometry, MeshStandardMaterial>): void => {
        memory.geometry.dispose();
        memory.material.dispose();
      }
    );
    this._paths.forEach(
      (path: LineSegments<BufferGeometry, LineBasicMaterial>): void => {
        path.geometry.dispose();
        path.material.dispose();
      }
    );
    this._pathEchoes.forEach(
      (echo: LineSegments<BufferGeometry, LineBasicMaterial>): void =>
