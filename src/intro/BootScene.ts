...
type IntroTestWindow = Omit<
  Window & typeof globalThis,
  "__THREE_GAME_TEST_HOOKS__" | "__THREE_GAME_DIAGNOSTICS__"
> & {
  __THREE_GAME_DIAGNOSTICS__?: {
    renderer: WebGLRenderer["info"];
...
export class BootScene {
  ...
  public init(): void {
    ...
    document
      ...
      .forEach((element: HTMLElement, index: number): void => {
        element.textContent = BOOT_OPTIONS[index];
      });
    ...
    try {
      ...
      this._textures = PLATE_URLS.map((url: string): Texture => {
        const texture: Texture = loader.load(
          ...
          (image: Texture): void => {
            ...
            loaded += 1;
            if (loaded !== PLATE_URLS.length || !this._plate || !this._root)
              return;
            if (!(image.image instanceof HTMLImageElement)) {
              this._showError(
                "The opening image could not decode. Reload to try again."
              );
              return;
            }
            this._plate.material.uniforms.imageAspect.value =
          ...
          () =>
            this._showError(
              "The opening image could not load. Reload to try again."
            )
        );
      ...
      const material: ShaderMaterial = new ShaderMaterial({
        uniforms: {
          distantImage: { value: this._textures[0] },
          foldImage: { value: this._textures[1] },
          thresholdImage: { value: this._textures[2] },
          firstMix: { value: 0 },
          secondMix: { value: 0 },
          zoom: { value: 1 },
          viewAspect: { value: 1 },
  ...
  private _update = (now: number): void => {
    ...
    if (!document.hidden) {
      ...
      const firstMix: number = this._isReduced
        ? 0
        : Math.min(
            Math.max((displayMs - FIRST_DISSOLVE_MS) / DISSOLVE_DURATION_MS, 0),
            1
          );
      const targetSecondMix: number =
        this._storyMode === "boot"
          ? 0
          : this._storyMode === "final" || this._storyMode === "complete"
            ? 1
            : this._questionIndex / Math.max(1, CHRONICLE_QUESTIONS.length - 1);
      this._secondMix +=
        (targetSecondMix - this._secondMix) *
        Math.min(1, delta * (this._isReduced ? 60 : 0.42));
      this._plate.material.uniforms.firstMix.value = firstMix;
      this._plate.material.uniforms.secondMix.value = this._secondMix;
      this._plate.material.uniforms.zoom.value = this._isReduced
        ? 1
        : 1 +
          Math.min(this._motionMs / ZOOM_DURATION_MS, 1) * 0.06 +
          this._questionIndex * 0.009;
      this._plate.material.uniforms.reveal.value = this._isReduced
        ? 0.7
        : Math.min(Math.max((displayMs - 4400) / 1600, 0), 0.85);
      this._plate.material.uniforms.drift.value.set(
        this._isReduced ? 0 : Math.sin(this._motionMs / 13000) * 0.019,
        this._isReduced ? 0 : Math.cos(this._motionMs / 19000) * 0.013
      );
      if (
        this._storyMode !== "boot" &&
      ...
      if (this._root)
        this._root.dataset.osPlateTs =
          this._secondMix > 0.98 ? "2" : firstMix === 1 ? "1" : "0";
  ...
  private _commitChoice(index: number): void {
    ...
    const question: ChronicleQuestion =
      CHRONICLE_QUESTIONS[this._questionIndex];
  ...
  private _advanceStory(): void {
    ...
    if (this._storyMode === "response") {
      if (this._questionIndex >= CHRONICLE_QUESTIONS.length - 1)
        this._beginFinal();
  ...
  private _updateStory(now: number): void {
    ...
    const question: ChronicleQuestion =
      CHRONICLE_QUESTIONS[
        Math.min(this._questionIndex, CHRONICLE_QUESTIONS.length - 1)
      ];
    if (this._storyMode === "prelude") {
      ...
      this._applyFrame(
        this._storyFrame(
          ...
          "prelude",
          question.era,
          complete ? "ENTER  //  LET IT ASK" : undefined,
          elapsedMs
        )
      );
      return;
    ...
    if (this._storyMode === "final") {
      ...
      this._applyFrame(
        this._storyFrame(
          ...
          4,
          complete ? "ALL THREE FOLLOWED" : undefined,
          elapsedMs
        )
      );
    }
  ...
  private _finalText(elapsedMs: number): string {
    ...
    if (elapsedMs < 3200)
      return CHRONICLE_FINAL_DRAFT.slice(0, Math.floor(elapsedMs / 55));
    if (elapsedMs < 4400) return CHRONICLE_FINAL_DRAFT;
    if (elapsedMs < 4800)
      return "[SYSTEM: Dreamweaver thread sele_ted - {{THREAD_NAME}}]";
    if (elapsedMs < 5200)
      return "[SYSTEM: Dreamweaver thread ________ - {{THREAD_NAME}}]";
    ...
    return (
      firstLine +
      CHRONICLE_FINAL.slice(
        firstLine.length,
        firstLine.length + Math.floor((elapsedMs - 5800) / 34)
      )
    );
  }
  ...
  private _onKeyDown = (event: KeyboardEvent): void => {
    ...
    if (
      this._storyMode === "waiting" &&
      ["ArrowDown", "ArrowUp", " ", "Enter", "1", "2", "3"].includes(event.key)
    ) {
      event.preventDefault();
      if (event.key === "ArrowDown")
        this._activeChoice = (this._activeChoice + 1) % 3;
      if (event.key === "ArrowUp")
        this._activeChoice = (this._activeChoice + 2) % 3;
  ...
  public destroy(): void {
    ...
    const testWindow: IntroTestWindow = window as IntroTestWindow;
  ...
  private _applyTestState(name: TestStateName): void {
    ...
    if (name.startsWith("question-")) {
      ...
      const question: ChronicleQuestion | undefined =
        CHRONICLE_QUESTIONS[questionIndex];
    ...
    if (name === "final-door") {
      this._storyMode = "final";
      this._applyFrame(
        this._storyFrame(CHRONICLE_FINAL_DRAFT, "final", 4, undefined, 4400)
      );
      return;
    }
    this._storyMode = "complete";
  ...
  private _getDiagnosticState(): IntroDiagnosticState {
    ...
    return {
      ...
      playerPosition: this._spatial.getPlayerPosition(this._activeChoice),
      seed: this._seed,
      audio: {
