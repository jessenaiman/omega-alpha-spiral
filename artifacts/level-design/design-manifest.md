...
| Floor 1 · Light    | One character; straight lines and right angles shape routes to Door, Monster, and Chest choices. Each choice has its own physical exit; crossing the chosen exit advances. Per-journey seeds permute Door, Monster, and Chest among three distinct slots on each floor, so doorway placement changes across runs while the floor's geometry remains recognizable. `?seed=N` reproduces a layout for review.    | Atari-era raster playfield logic extruded into sparse physical 3D. The [approved Stage 1 starting point](stage-1-atari-3d-starting-point.png) sets density.                                                                                                                          |
...
Next review: use the [two-seed early-floor bot video](../qa-61/seeded-exit-bot.webm) and [Shadow action frames](../qa-61/shadow-action-seed-17.png) to review the playable geometry. Tune the camera and HUD so exits stay visible and the playfield fills the frame, then tune Stage 7's mixed display proportions in play. The bot establishes route mechanics, not aesthetic approval. Use the existing studio and v1 schema for the smallest scene-effect and NPC extension with the owner.
