...
`npm run typecheck`, `npm run test:unit` (76 pass), `npm run test:browser`
(9 pass), `npm run build`, the production preview check, and
`check_evidence.py --manifest` (16 artifacts, pass-3) all pass. See
...
- Tests: `tests/unit/spiral-breaker-rules.test.ts`,
  `tests/unit/bot-playtest.test.ts`, `tests/browser/spiral-breaker.spec.ts`,
  `tests/browser/qa-release.spec.ts`
