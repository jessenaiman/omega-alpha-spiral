...
async function prepareDeterministicScreenshot(page: Page, stateName: string) {
  ...
  const acknowledgement = await page.evaluate(async (name) => {
    ...
    if (!hooks)
      throw new Error(
        "__THREE_GAME_TEST_HOOKS__ is required for visual baselines"
      );
    for (const key of [
      "seed",
      "setState",
      ...
      if (typeof hooks[key] !== "function")
        throw new Error(`Visual baselines require the ${key} hook`);
    }
    await hooks.setPausedForScreenshot(false);
    await hooks.seed(12345);
    const applied = await hooks.setState(name);
    if (!applied || applied.state !== name)
      throw new Error(
        `setState must acknowledge {state: ${JSON.stringify(name)}}`
      );
  ...
  expect(acknowledgement.state, "requested visual state must be applied").toBe(
    stateName
  );
}
...
test("active play visual baseline", async ({ page }, testInfo) => {
  ...
  await expect(page).toHaveScreenshot(
    `active-play-${testInfo.project.name}.png`,
    {
      fullPage: true,
