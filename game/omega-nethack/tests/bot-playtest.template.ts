...
test("bot playtest: scripted input drives progress without errors", async ({
  ...
  const acknowledgement = await page.evaluate(async () => {
    ...
    if (!applied || applied.state !== "active-play")
      throw new Error("setState must acknowledge active-play");
    return applied;
  });
  ...
  expect(
    before,
    "diagnostics must be published before the bot can play"
  ).not.toBeNull();
  ...
  for (const [index, step] of INPUT_SCRIPT.entries()) {
    ...
    if (snap) {
      ...
      distance += moved;
      const progressed =
        snap.score > prev.score || (snap.complete && !prev.complete);
      ...
      if (snap.frame > prev.frame && moved < 0.2 && !progressed)
        softlockWindows += 1;
      snapshots.push(snap);
    }
  }
